...
contract BorrowTests is Base {
    ...
    function test_borrowFromActiveTrove_multipleAuctions(
        ...
        vm.prank(userBorrower);
        troveManager.borrow(_troveId, _expectedDebt, type(uint256).max);
        ...
        uint256 _thirdCollateralNeeded =
            (troveManager.troves(_troveId).debt * DEFAULT_TARGET_COLLATERAL_RATIO / BORROW_TOKEN_PRECISION) * ORACLE_PRICE_SCALE / priceOracle.get_price() * 2;
        ...
        troveManager.borrow(_thirdTroveId, troveManager.troves(_troveId).debt, type(uint256).max);
        vm.stopPrank();

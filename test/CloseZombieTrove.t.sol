...
contract CloseZombieTroveTests is Base {
    ...
    function test_closeZombieTrove(uint256 _amount) public {
        ...
        assertApproxEqRel(borrowToken.balanceOf(address(userLender)), _amountToPull, 3e16, "E23"); // 3%. Slippage

        // Do some intermediate checks on borrower
        _trove = troveManager.troves(_troveId);
        assertEq(uint256(_trove.status), uint256(ITroveManager.Status.zombie), "E24");
        assertEq(_trove.debt, _expectedDebt - _amountToPull, "E25");
        assertEq(troveManager.zombie_trove_id(), _troveId, "E26");
        ...
        assertEq(uint256(_trove.status), 4, "E33"); // Closed
        ...
        assertEq(collateralToken.balanceOf(address(userBorrower)), _expectedCollateralAfterRedemption, "E41");
        assertEq(borrowToken.balanceOf(address(troveManager)), 0, "E42");
        assertGe(borrowToken.balanceOf(address(lender)), _newExpectedDebt, "E43");
        assertEq(borrowToken.balanceOf(userBorrower), 0, "E44");
    ...
    function test_closeZombieTrove_zeroDebt(uint256 _amount) public {
        ...
        // Check exchange is empty
        assertEq(borrowToken.balanceOf(address(exchange)), 0, "E73");
        assertEq(collateralToken.balanceOf(address(exchange)), 0, "E74");
    }
}

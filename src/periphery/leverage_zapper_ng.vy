...
def onMorphoFlashLoan(
    ...
    elif operation == Operation.SETTLE:
        flash_loan_token = self._handle_settle(assets, data)
    else:
        raise "!operation"
...
def _handle_open(flash_loan_amount: uint256, data: Bytes[_MAX_FLASHLOAN_CALLBACK_DATA_SIZE]) -> address:
    ...
    # Settle the auction if one was kicked
    self._settle_auction(
        params.taker_swap,
        dutch_desk,
        params.auction_taker,
        params.flash_loan_token,
        collateral_token,
        borrow_token,
        nonce_before,
        params.debt_amount,
        params.taker_funding,
    )
...
@internal
def _handle_settle(flash_loan_amount: uint256, data: Bytes[_MAX_FLASHLOAN_CALLBACK_DATA_SIZE]) -> address:
    """
    @notice Settle a redemption auction inside a nested borrow-token flash loan without converting collateral
    @dev Pays the auction directly with the flash loaned borrow token
    @param flash_loan_amount The amount of borrow token flash loaned (>= the auction's needed amount)
    @param data The encoded parameters
    @return The flash loan token address (the borrow token)
    """
    # Decode parameters
    operation: Operation = empty(Operation)
    borrow_token: address = empty(address)
    auction: address = empty(address)
    auction_id: uint256 = 0
    operation, borrow_token, auction, auction_id = abi_decode(data, (Operation, address, address, uint256))

    # Approve the auction to pull the needed borrow tokens
    assert extcall IERC20(borrow_token).approve(auction, flash_loan_amount, default_return_value=True)

    # Take the whole auction with no callback as we already hold the borrow token to pay
    extcall IAuction(auction).take(auction_id, max_value(uint256), self, b"")

    # Reset any residual approval
    assert extcall IERC20(borrow_token).approve(auction, 0, default_return_value=True)
...
def _settle_auction(
    ...
    @param flash_loan_token The token flashloaned by the outer operation
    @param collateral_token The market collateral token
    @param borrow_token The market borrow token
    @param nonce_before The Dutch Desk nonce before the borrow
    @param debt_amount The Trove debt borrowed. >= the auction's needed amount, sizes the settle flashloan
    @param taker_funding The amount forwarded to the taker (taker path only)
    """
    # Return early if no auction was kicked
    if staticcall dutch_desk.nonce() <= nonce_before:
        return

    auction: address = staticcall dutch_desk.auction()
    if auction_taker != empty(address):
        # If provided, fund the auction taker
        if taker_funding > 0:
            assert extcall IERC20(flash_loan_token).transfer(auction_taker, taker_funding, default_return_value=True)

        # Take the auction
        extcall IZapperAuctionTakerNG(auction_taker).takeAuction(
            auction,
            nonce_before,
            taker_swap.router,
            taker_swap.data,
        )
    elif flash_loan_token == collateral_token:
        # Make sure there is no starting price buffer
        assert staticcall dutch_desk.starting_price_buffer_percentage() <= _MAX_STARTING_PRICE_BUFFER, "!buffer"

        # Flashloan the borrow token and take the auction via the `_handle_settle()` callback
        extcall _MORPHO.flashLoan(
            borrow_token,
            debt_amount,
            abi_encode(Operation.SETTLE, borrow_token, auction, nonce_before),
        )
    else:
        # Cannot settle the auction
        raise "!settle"

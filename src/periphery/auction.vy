...
# @todo -- test nonreentrancy functionality
# @todo -- here -- finish fixing tests and create new test for this contract
"""
@title Auction
@license MIT
@author Flex
...
def get_needed_amount(
    ...
    """
    @notice Calculate the amount of want needed to buy `max_take_amount`
...
def is_ongoing_liquidation_auction() -> bool:
    """
    @notice Check if there is at least one ongoing liquidation auction
    @return Whether there is at least one ongoing liquidation auction
    """
...
def re_kick(
    ...
    @param minimum_price The new minimum price for the auction (scaled to 1e18)
    """
    # Make sure caller is Papi
    assert msg.sender == PAPI, "!papi"
...
def take(
    ...
    @param data The data to pass to the taker callback
    ...
    needed_amount: uint256 = self._get_amount_needed(auction, take_amount, block.timestamp)
...
def _get_amount_needed(
    ...
    """
    @notice Calculate the amount of want token needed to buy `take_amount` of from token
...
@internal
@view
def _get_price(auction: AuctionInfo, at_timestamp: uint256) -> uint256:
    """
...
def _is_active(auction: AuctionInfo) -> bool:
    ...
    @return Whether the auction is active
    """
    return self._get_price(auction, block.timestamp) > 0
